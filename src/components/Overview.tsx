...
const Overview: React.FC<OverviewProps> = ({
  ...
  // Օժանդակ ֆունկցիա՝ առարկայի ընդհանուր ժամաքանակը հաշվելու համար
  const getTotalHours = (subjectHours: { [id: string]: number }) =>
    Object.values(subjectHours).reduce((s, h) => s + h, 0);

  // Տվյալների վավերացում և հնարավոր խնդիրների ֆիլտրում
  ...
  // Ստուգողական ցուցակ (Checklist)՝ դասացուցակի պատրաստվածությունը գնահատելու համար
  const checklist = [
    {
  ...
  // Խմբերի խմբավորում ըստ մասնագիտացումների
  ...
  // Ուսուցիչների ծանրաբեռնվածության և շաբաթական ժամերի հաշվարկ
  ...
  // Կոնֆիգուրացիայի ֆայլի ներմուծման (Import) մշակում
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      {/* Գլխամաս (Header) — Լիովին համապատասխանում է մյուս բաժինների ոճին */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* ՁԱԽ ԿՈՂՄ — Ստուգողական ցուցակ + Առկա խնդիրներ */}
        <div className="space-y-4">
          {/* Ստուգողական ցուցակի վահանակ */}
          ...
          {/* Խնդիրների արտացոլման վահանակ (Key Issues) */}
        ...
        {/* ԱՋ ԿՈՂՄ — Տվյալների տեղեկատվական վահանակներ */}
        <div className="lg:col-span-2 space-y-4">
          {/* 4 հիմնական վիճակագրական քարտեր (Stat cards) */}
          ...
          {/* Խմբերի բաշխվածությունը ըստ մասնագիտացումների (Specialization summary) */}
          ...
          {/* Ուսուցիչների աշխատանքային ծանրաբեռնվածության ցուցակ (Teacher workload) */}
          {teachers.length > 0 && (
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
      ...
      {/* Տվյալների ջնջման հաստատման մոդալ պատուհան (Delete Confirm Modal) */}
      {showDeleteConfirm && (
