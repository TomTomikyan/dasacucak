...
// Իկոնաների ներմուծում lucide-react գրադարանից գրաֆիկական ինտերֆեյսի համար
import { Plus, GraduationCap, Trash2, Clock, BookOpen, MapPin, CreditCard as Edit, Save, X, Users } from 'lucide-react';
// Տվյալների տիպերի ներմուծում props-երի և օբյեկտների խիստ տիպայնացման համար
import { Teacher, Subject, Classroom, ClassGroup, Institution } from '../types';
// Լոկալիզացիայի (բազմալեզվության) աջակցման քասթոմ հուկ
import { useLocalization } from '../hooks/useLocalization';

// Teachers կոմպոնենտի props-երի ինտերֆեյսը
interface TeachersProps {
  teachers: Teacher[]; // Բոլոր դասախոսների ցուցակը
  addTeacher: (teacher: Omit<Teacher, 'id'>) => void; // Նոր դասախոս ավելացնելու ֆունկցիա
  setTeachers: (teachers: Teacher[]) => void; // Դասախոսների ցուցակը թարմացնելու ֆունկցիա
  subjects: Subject[]; // Հասանելի առարկաների ցուցակը
  classrooms: Classroom[]; // Լսարանների ցուցակը
  classGroups: ClassGroup[]; // Ուսանողական խմբերի ցուցակը
  institution: Institution; // Ուսումնական հաստատության տվյալները (աշխատանքային օրեր, ժամեր)
  showToast: { // Ծանուցումների (toast) ցուցադրման օբյեկտ
...
/**
 * Tooltip կոմպոնենտ (Հուշում)
 * Օգտագործում է ֆիքսված դիրքավորում (fixed) կուրսորի կոորդինատների նկատմամբ,
 * որպեսզի խուսափի overflow ունեցող աղյուսակների ներսում տեքստի կտրվելուց:
 */
const Tooltip: React.FC<{ content: string; children: React.ReactNode }> = ({ content, children }) => {
  const [isVisible, setIsVisible] = useState(false); // Հուշման տեսանելիությունը
  const [position, setPosition] = useState({ x: 0, y: 0 }); // Ցուցադրման կոորդինատները

  // Մկնիկի մոտեցման մշակող․ հաշվարկում է տարրի կենտրոնը հուշման ճշգրիտ դիրքավորման համար
  const handleMouseEnter = (e: React.MouseEvent) => {
    const rect = e.currentTarget.getBoundingClientRect();
    setPosition({
      x: rect.left + rect.width / 2,
      y: rect.top - 10 // Տեղաշարժ տարրից մի փոքր վերև
    });
    setIsVisible(true);
  };
  ...
  return (
    ...
    >
      ...
      {isVisible && (
        ...
        >
          {content}
          {/* Փոքրիկ սլաք-ցուցիչ հուշման ներքևում */}
...
/**
 * Դասախոսական կազմի կառավարման հիմնական Teachers կոմպոնենտը
 */
const Teachers: React.FC<TeachersProps> = ({
  ...
  const { t } = useLocalization(); // Հուկ թարգմանությունների համար
  
  // Ինտերֆեյսի վիճակների (states) կառավարում
  const [showForm, setShowForm] = useState(false); // Ձևաթղթի (form) մոդալ պատուհանի ցուցադրում/թաքցնում
  const [editingTeacher, setEditingTeacher] = useState<Teacher | null>(null); // Պահում է խմբագրվող դասախոսին
  const [deletingTeacher, setDeletingTeacher] = useState<Teacher | null>(null); // Պահում է հեռացման համար ընտրված դասախոսին
  const [showValidationError, setShowValidationError] = useState(false); // Վավերացման սխալի դրոշակ (օրինակ՝ եթե խմբեր ընտրված չեն)
  
  /**
   * Գեներացնում է լռելյայն հասանելի ժամերի օբյեկտը՝ հիմնվելով հաստատության կարգավորումների վրա:
   * Բոլոր աշխատանքային օրերը լրացնում է դասաժամերի ամբողջական ցուցակով:
   */
  ...
  // Ձևաթղթի տվյալների վիճակը (ավելացման և խմբագրման համար)
  const [formData, setFormData] = useState({
    firstName: '',
    lastName: '',
    subjects: [] as string[], // Առարկաների անունների ցուցակը, որոնք դասավանդում է դասախոսը
    homeClassroom: '', // Դասախոսին կցված լսարանը (ID)
    availableHours: getDefaultAvailableHours(), // Հասանելիության գրաֆիկն ըստ օրերի և ժամերի
    assignedClassGroups: [] as string[], // Ուսանողական խմբերը, որոնցում նա դասեր է վարում
  });

  // Դասաժամերի համարների զանգված (օրինակ՝ [1, 2, 3, 4, 5, 6]) չեկբոքսերի արտացոլման համար
  const lessonHours = Array.from({ length: institution.lessonsPerDay }, (_, i) => i + 1);

  /**
   * Վերադարձնում է դասախոսին կցելու համար հասանելի լսարանների ցուցակը:
   * Ֆիլտրում է միայն 'teacher_lab' տիպի լսարանները և բացառում նրանք, որոնք արդեն զբաղված են այլ դասախոսների կողմից:
   */
  ...
  // Լսարանի տեքստային անվան ստացում (համար և հարկ) ըստ ID-ի
  const getClassroomName = (classroomId: string) => {
    const classroom = classrooms.find(c => c.id === classroomId);
    return classroom ? `${classroom.number} (${t('common.floor')} ${classroom.floor})` : t('common.unknown');
  };

  // Խմբի անվան ստացում ըստ ID-ի
  const getGroupName = (groupId: string) => {
    const group = classGroups.find(g => g.id === groupId);
    return group ? group.name : t('common.unknown');
  };

  // Կուրսի համարի թարգմանության ստացում
  const getCourseText = (courseNumber: number) => {
    return t(`courses.${courseNumber}`);
  };

  /**
   * Փոխակերպում է դասախոսի առարկաների ID-ների/անունների զանգվածը արդիական անունների:
   * Օգտագործվում է սինխրոնիզացիայի համար, եթե առարկաների տվյալները փոխվել են useScheduleData-ում:
   */
  ...
  /**
   * Ձևաթուղթը փոխանցում է գոյություն ունեցող դասախոսի խմբագրման ռեժիմի:
   * Լրացնում է formData-ն ընթացիկ արժեքներով և վավերացնում է ժամերը՝ ըստ հաստատության ընթացիկ կարգավորումների:
   */
  const startEditing = (teacher: Teacher) => {
    setEditingTeacher(teacher);
    setShowValidationError(false);

    // Դասախոսի հասանելի ժամերի համապատասխանության ստուգում հաստատության ընթացիկ կարգավորումներին
    const updatedAvailableHours = { ...teacher.availableHours };
    institution.workingDays.forEach(day => {
      if (!updatedAvailableHours[day]) {
        // Եթե համակարգում նոր աշխատանքային օր է ավելացել, այն դասախոսին ավելացնում ենք որպես ամբողջությամբ հասանելի
        updatedAvailableHours[day] = Array.from({ length: institution.lessonsPerDay }, (_, i) => i + 1);
      } else {
        // Կտրում ենք ժամերը, եթե օրական առավելագույն դասաժամերի քանակը նվազել է կարգավորումներում
  ...
  // Ձևաթղթի մաքրում և մոդալ պատուհանի փակում
  const cancelEdit = () => {
  ...
  /**
   * Ձևաթղթի ուղարկման մշակող (Տվյալների ավելացում կամ թարմացում)
   */
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();

    // Պարտադիր դաշտերի վավերացում: Անուն և Ազգանուն
    ...
    // Վավերացում: դասախոսին պարտադիր պետք է կցված լինի առնվազն մեկ խումբ
    ...
    if (editingTeacher) {
      // Զանգվածում գոյություն ունեցող դասախոսի թարմացման տրամաբանությունը
      const updatedTeachers = teachers.map(teacher =>
        teacher.id === editingTeacher.id
          ? {
              ...
              homeClassroom: formData.homeClassroom || undefined, // Պահպանում ենք undefined, եթե լսարանը ընտրված չէ
      ...
      // Նոր դասախոս ստեղծելու տրամաբանությունը
      addTeacher({
        ...formData,
    ...
    // Ձևաթղթի վիճակի մաքրում հաջող գործողությունից հետո
    setFormData({
      firstName: '',
      lastName: '',
      subjects: [],
  ...
  // Առարկաների ընտրության փոխարկում (ավելացում/հեռացում չեկբոքսերի զանգվածից)
  ...
  // Խմբերի ընտրության փոխարկում և ընտրության ժամանակ վավերացման սխալի ավտոմատ մաքրում
  ...
  // Հասանելի ժամերի կառավարում (կոնկրետ դասաժամի ընտրություն շաբաթվա կոնկրետ օրը)
  const handleAvailableHourToggle = (day: string, hour: number, checked: boolean) => {
    setFormData({
      ...
      availableHours: {
        ...
        [day]: checked
          ? [...formData.availableHours[day], hour].sort((a, b) => a - b) // Սորտավորում ենք ժամերն ըստ աճման
  ...
  /**
   * Դասախոսի ԱԱՀ-ով վանդակի հուշման բովանդակության ձևավորում
   */
  ...
  // Հուշում առարկաների ցուցակի համար
  ...
  // Հուշում կցված խմբերի ցուցակի համար (կուրսի և ուսանողների քանակի ցուցադրմամբ)
  ...
  // Հուշում կցված լսարանի մասին մանրամասն տեղեկություններով
  ...
  // Հուշում՝ ըստ շաբաթվա օրերի հասանելի աշխատանքային ժամերի մանրամասնմամբ
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {/* Ավելացման / խմբագրման ձևաթղթի մոդալ պատուհան */}
      {showForm && (
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <form onSubmit={handleSubmit} className="p-6">
              ...
              <div className="space-y-6">
                {/* Հիմնական տեղեկատվություն: Անուն և Ազգանուն */}
                ...
                {/* Առարկաների ընտրություն (չեկբոքսերի ցուցակ) */}
                {subjects.length > 0 && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                ...
                {/* Կցված խմբերի ընտրություն (Պարտադիր դաշտ) */}
                {classGroups.length > 0 ? (
                  <div>
                    ...
                    {/* Դինամիկ դաս: եթե կա վավերացման սխալ, բլոկը լուսավորվում է կարմիր գույնով */}
                    ...
                    {/* Սխալների կամ խմբերի ընտրության կարգավիճակի մասին հաղորդագրություններ */}
                    {showValidationError && formData.assignedClassGroups.length === 0 ? (
                ...
                {/* Հասանելի աշխատանքային ժամերի ընտրության ցանց (Շաբաթվա օրեր / Դասաժամերի համարներ) */}
                <div>
              ...
              {/* Ձևաթղթի կառավարման կոճակներ (Չեղարկել / Պահպանել) */}
      ...
      {/* Հեռացման հաստատման մոդալ պատուհան */}
      {deletingTeacher && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        {teachers.length === 0 ? (
          ...
          /* Սքրոլբար (Scrollbar) աղյուսակի համար՝ ֆիքսված առավելագույն բարձրությամբ */
          <div className="overflow-x-auto" style={{ maxHeight: 'calc(100vh - 220px)', overflowY: 'auto' }}>
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('common.teacher')}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('subjects.title')}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('teachers.assignedGroups')}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('teachers.ownClassroom')}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('teachers.availableDays')}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('teachers.weeklyHours')}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('common.actions')}</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {teachers.map((teacher) => {
                  // Հաշվարկում ենք ընթացիկ դասախոսի շաբաթական հասանելի ժամերի ընդհանուր քանակը
                  const totalHours = Object.values(teacher.availableHours).reduce((sum, hours) => sum + hours.length, 0);
                  const currentSubjectNames = getTeacherSubjectNames(teacher.subjects);
                  
                  return (
                    <tr key={teacher.id} className="hover:bg-gray-50">
                      {/* Վանդակ: Դասախոսի անունը + սկզբնատառերով կլոր ավատար */}
                      ...
                      {/* Վանդակ: Առարկաներ (ցույց է տալիս առաջին 2-ը, մնացածը թաքցնում է "+X"-ի տակ) */}
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {/* Վանդակ: Կցված խմբեր (ցուցադրում է առաջին 3-ը) */}
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {teacher.homeClassroom ? (
                          ...
                          <Tooltip content="Սեփական դասարան չունի (Нет личной аудитории)">
                            <span className="text-gray-400 italic cursor-help">չունի</span>
                          </Tooltip>
                        )}
                      </td>

                      {/* Վանդակ: Հասանելի օրերի քանակը */}
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      ...
                      {/* Վանդակ: Շաբաթական ընդհանուր աշխատանքային ժամեր */}
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {/* Գործողությունների վանդակ: Խմբագրում և Հեռացում */}
