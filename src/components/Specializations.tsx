...
// Prop-երի տիպայնացում Specializations կոմպոնենտի համար
...
const Specializations: React.FC<SpecializationsProps> = ({
  ...
  // --- Լոկալ Սթեյթներ (State Management) ---
  const [showForm, setShowForm] = useState(false); // Ավելացման/Խմբագրման մոդալ պատուհանի տեսանելիությունը
  const [editingSpec, setEditingSpec] = useState<Specialization | null>(null); // Ընթացիկ խմբագրվող մասնագիտությունը
  const [deletingSpec, setDeletingSpec] = useState<Specialization | null>(null); // Ջնջման ենթակա մասնագիտությունը

  // Ֆորմայի դատարկ/սկզբնական վիճակը գեներացնող ֆունկցիա
  const emptyForm = () => ({
    code: '',
    name: '',
    course: 1,
    academicWeeks: institution.academicWeeks || 36, // Վերցնում է քոլեջի լռելյայն շաբաթները
    subjectHours: {} as { [subjectId: string]: number }, // Առարկաների ID-ներն ու դրանց ժամերը
  });

  const [formData, setFormData] = useState(emptyForm()); // Ֆորմայի ընթացիկ տվյալները
  const [subjectSearch, setSubjectSearch] = useState(''); // Առարկաների որոնման տեքստը
  const [showSubjectDropdown, setShowSubjectDropdown] = useState(false); // Առարկաների dropdown-ի տեսանելիությունը
  const searchRef = useRef<HTMLDivElement>(null); // Հղում որոնման բլոկին՝ դրսից սեղմումները որսալու համար

  // --- Էֆեկտներ (Effects) ---
  // Փակում է առարկաների dropdown-ը, եթե օգտատերը սեղմում է դրանից դուրս
  useEffect(() => {
  ...
  // Ավելացված առարկաների ID-ների հավաքագրում
  ...
  // Որոնման ենթակա առարկաների ֆիլտրում (ըստ անվանման, ընթացիկ կուրսի և որոնք դեռ ավելացված չեն)
  const searchableSubjects = subjects.filter((s) => {
    ...
    const matchesCourse = s.course === formData.course; // Միայն տվյալ կուրսի առարկաները
    return !alreadyAdded && matchesSearch && matchesCourse;
  });

  // Ֆորմայում արդեն իսկ ավելացված առարկաների ամբողջական օբյեկտները
  const addedSubjects = subjects.filter((s) => s.id in formData.subjectHours);

  // Վալիդացիա՝ հնարավո՞ր է պահպանել ֆորման (կոդը, անունը և առնվազն 1 առարկա ժամով լրացված պետք է լինեն)
  const canSubmit =
    formData.code.trim().length > 0 &&
    formData.name.trim().length > 0 &&
    Object.values(formData.subjectHours).some((h) => h > 0);

  // --- Ֆունկցիաներ և Իրադարձություններ (Handlers) ---
  // Առարկայի ժամաքանակի փոփոխում կամ հեռացում
  const setSubjectHour = (subjectId: string, val: number) => {
    if (val < 0) {
      // Եթե արժեքը -1 է, հեռացնում ենք առարկան ցուցակից
      const copy = { ...formData.subjectHours };
      delete copy[subjectId];
      setFormData({ ...formData, subjectHours: copy });
    } else {
      // Հակառակ դեպքում թարմացնում ենք ժամաքանակը
      setFormData({ ...formData, subjectHours: { ...formData.subjectHours, [subjectId]: val } });
    }
  };

  // Ավելացման մոդալ պատուհանի բացում
  const openAdd = () => {
    setEditingSpec(null);
  ...
  // Խմբագրման մոդալ պատուհանի բացում՝ գոյություն ունեցող տվյալների լրացմամբ
  ...
  // Մոդալ ֆորմայի փակում և մաքրում
  const closeForm = () => {
    setShowForm(false);
    setEditingSpec(null);
    setFormData(emptyForm());
  ...
  // Ֆորմայի հաստատում (Submit)՝ Ավելացում կամ Խմբագրում
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    
    // Վալիդացիայի ստուգում և համապատասխան Warning Toast-երի ցուցադրում
    if (!canSubmit) {
    ...
    if (editingSpec) {
      // Գոյություն ունեցող մասնագիտության թարմացում
      setSpecializations(
  ...
  // Օգնող ֆունկցիա՝ ID-ով առարկայի անունը ստանալու համար
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <form onSubmit={handleSubmit} className="p-6">
              ...
              <div className="space-y-5">
                ...
                {/* Կուրս և Ուսումնական Շաբաթներ դաշտեր */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...
                          subjectHours: {}, // Կուրսը փոխելիս առարկաները մաքրվում են (քանի որ դրանք կապված են կուրսի հետ)
                        })
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#03524f]"
                    />
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {/* Վերականգնելու կոճակ, եթե շաբաթների քանակը տարբերվում է հաստատության լռելյայն արժեքից */}
                ...
                <div>
                  ...
                  {subjects.length === 0 ? (
                    ...
                    <>
                      {/* Որոնում և Dropdown-ից ընտրություն */}
                            ...
                            {searchableSubjects.length === 0 ? (
                              ...
                              searchableSubjects.map((subj) => (
                                <button
                                  ...
                                  type="button"
                                  onClick={() => {
                                    setSubjectHour(subj.id, 0); // Սկզբնական ավելացնում ենք 0 ժամով
                                    setSubjectSearch('');
                                    setShowSubjectDropdown(false);
                                  }}
                      ...
                      {/* Արդեն իսկ ավելացված առարկաների ցուցակը ժամերով */}
                        ...
                        <div className="border border-gray-300 rounded-md p-3 max-h-52 overflow-y-auto space-y-2">
                          {addedSubjects.map((subj) => (
                            <div key={subj.id} className="flex items-center gap-3">
                              ...
                              <button
                                type="button"
                                onClick={() => setSubjectHour(subj.id, -1)} // Հեռացնում է առարկան ցուցակից
              ...
              {/* Ֆորմայի Գործողությունների Կոճակներ */}
      ...
      {/* --- Ջնջման Հաստատման Մոդալ Պատուհան --- */}
      {deletingSpec && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      ...
      {/* --- Մասնագիտությունների Հիմնական Ցուցակ / Աղյուսակ --- */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        {specializations.length === 0 ? (
          // Դատարկ վիճակի (Empty State) ցուցադրում
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {specializations.map((spec) => {
                  // Տվյալ մասնագիտությանը կցված առարկաների և ընդհանուր ժամերի հաշվարկ
                  ...
                  return (
                    <tr key={spec.id} className="hover:bg-gray-50">
                      ...
                      {/* Առարկաների պիտակներ (badges) - Ցուցադրվում է առավելագույնը 3 հատ */}
                      ...
                      {/* Գործողությունների կոճակներ (Խմբագրել / Ջնջել) */}
